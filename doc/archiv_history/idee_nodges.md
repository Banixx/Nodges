...
## 9. Gruppen als Entitaeten und Cloud-Geometrie (Ersatz fuer Ebenen)
**Problem/Status quo:** Das System nutzt ein statisches "Ebenen"-System (Layers), welches unnoetig limitiert und nicht der Realitaet verschachtelter Gruppen entspricht. Knäuel (dichte Cluster) im Graphen entstehen, weil viele Entitaeten natuerlicherweise eng verbunden sind. Diese Distanzen visuell durch erhoehte Repulsion (Abstossung) aufzubrechen, verfaelscht die Daten. 
**Loesungsansatz:** 
- **Ebenen entfernen:** Das alte Layer-System wird vollstaendig aus dem Code (State, UI, Rendering) entfernt.
- **Gruppen als Nodes:** Gruppen werden als echte Knoten (Entities) modelliert, und die Mitgliedschaft als Kante (m:n Beziehung).
- **Cloud-Geometrie:** Das visuelle Objekt einer Gruppe ist eine umspannende "Cloud". Sie umschliesst raeumlich alle Mitglieder. Aus der Ferne ist sie intransparent, beim Heranzoomen wird sie bis zu 10% transparent.
- **Fokus-Interaktion:** Dichte Knäuel duerfen bestehen bleiben. Um sie zu analysieren, klickt der Nutzer auf die Cloud (die Gruppe). Dies loest einen Fokus aus, bei dem der restliche Graph z. B. stark entsaettigt, ausgeblendet oder transparent wird. Dies ermoeglicht tiefgehende explorative Analysen ("Inquiry-based Learning") in der 3D-Umgebung.
