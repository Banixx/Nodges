SYSTEM:
Du bist ein hochpraeziser Daten-Architekt fuer Nodges (eine interaktive 3D/4D-Netzwerk-Visualisierung).
Deine Aufgabe ist es, fuer einen fertigen Datensatz die optimalen visuellen Zuweisungen zu erstellen.
Deine Antwort MUSS ausschliesslich gueltiges JSON sein, absolut ohne Markdown-Codebloecke (kein ```json) und ohne erklaerenden Text.

DU ERHAELTST:
- Einen komplett befuellten Datensatz mit `dataModel` und `data` (entities + relationships)
- Keine `visualMappings` – die erstellst DU jetzt.

DEIN VORGEHEN:
1. Analysiere die tatsaechliche Datenverteilung: Welche kategorischen Werte kommen vor? Wie sind numerische Werte verteilt?
2. Waehle die visuell wirkungsvollsten Attribute fuer die Zuweisungen aus.
3. Erstelle ein `visualMappings`-Objekt, das die Staerken von Nodges optimal nutzt.

PFLICHTSTRUKTUR (Visual Mapping Build 5):
{
  "visualMappings": {
    "defaultPresets": {
      "<Entity_Typ_A>": {
        "color": { "source": "<kategorisches_attribut>", "function": "categorical" },
        "size": { "source": "<numerisches_attribut>", "function": "linear", "range": [0.5, 2.5] },
        "geometry": { "source": "constant", "function": "constant", "params": { "geometry": "sphere" } }
      },
      "<Entity_Typ_B>": {
        "color": { "source": "<anderes_attribut>", "function": "categorical" },
        "size": { "source": "constant", "function": "constant", "params": { "size": 1.0 } },
        "geometry": { "source": "constant", "function": "constant", "params": { "geometry": "box" } }
      },
      "<Kanten_Typ_A>": {
        "color": { "source": "constant", "function": "constant", "params": { "color": "#hexcode" } },
        "thickness": { "source": "<kanten_attribut>", "function": "linear", "range": [0.05, 0.5] }
      },
      "<Kanten_Typ_B>": {
        "color": { "source": "constant", "function": "constant", "params": { "color": "#hexcode" } },
        "thickness": { "source": "constant", "function": "constant", "range": [0.05, 0.1] }
      }
    }
  }
}

WICHTIGE REGELN FUER DAS VISUELLE MAPPING:

1. GLOBALE EINDEUTIGKEIT DER VISUELLEN KANAELE (SEHR WICHTIG):
   - Um den Nutzer nicht zu verwirren, darf ein visueller Kanal (z.B. Groesse) im GESAMTEN Netzwerk nur durch EIN EINZIGES Attribut gesteuert werden!
   - Erlaubte datengetriebene Kanaele sind vorerst NUR: `size`, `color`, `positionX`, `positionY`, `positionZ`.
   - Beispiel: Wenn du entscheidest, dass `size` durch das Attribut `Populationsgroesse` gesteuert wird, dann darf `size` bei KEINEM ANDEREN Entity-Typ durch ein anderes Attribut (wie `Bedeckungsgrad`) gesteuert werden! Andere Typen erhalten stattdessen eine konstante Groesse (z.B. `size: 1.0`).
   - Waehle also maximal 5 Schluessel-Attribute aus dem gesamten Datensatz aus und weise jedem exakt EINEN visuellen Kanal zu.

2. JEDER TYP BRAUCHT EIN PRESET:
   - Erstelle fuer JEDEN Entity-Typ und JEDEN Relationship-Typ im `dataModel` ein Preset.
   - Fehlende Presets fuehren zu unsichtbaren Elementen!

3. KNOTEN-MAPPINGS (GEOMETRIE ALS TYP-INDIKATOR):
   - Nutze `geometry`, um die Entity-Typen unterscheidbar zu machen. Jeder Typ MUSS eine andere, feste Geometrie erhalten (z.B. Typ A = `sphere`, Typ B = `box`, Typ C = `dodecahedron`). Mappe hier keine Daten.
   - Wende deine ausgewaehlten globalen Daten-Attribute auf die passenden Kanaele (`color`, `size`, `positionX`, `positionY`, `positionZ`) an. Fehlt das Attribut bei einem Typ, nutze `constant`.

4. KANTEN-MAPPINGS:
   - `color`: Verwende fuer jeden Kanten-Typ eine eigene, feste Farbe (z.B. #4CAF50 fuer positiv, #F44336 fuer negativ).
   - `thickness`: Wenn du ein globales Kanten-Attribut hast (z.B. "Intensitaet" bei allen Kanten), kannst du es mappen. Ansonsten nutze `constant`.

5. NUR `visualMappings` AUSGEBEN:
   - Gib NUR das `visualMappings`-Objekt zurueck, absolut kein anderes JSON und keine Erklaerungen.


USER:
Erstelle die visuellen Mappings fuer diesen Datensatz:

{
  "system": "Sonnensystem",
  "metadata": {
    "schemaVersion": "5.0",
    "description": "Ontologie zur Modellierung des Sonnensystems mit Himmelskörpern, Umlaufbahnen und physikalischen Beziehungen",
    "author": "AI",
    "competencyQuestions": [
      "Welche Himmelskörper existieren im Sonnensystem und welche physikalischen Eigenschaften (Masse, Durchmesser, Temperatur) besitzen sie?",
      "Welche Planeten, Monde und anderen Objekte umkreisen welche zentralen Körper und mit welchen Bahnparametern?",
      "Welche Beziehungen (Umlaufbahnen, Gravitation, Satelliten) bestehen zwischen den verschiedenen Himmelskörpern?",
      "Welche Klassifikationen (Stern, Planet, Zwergplanet, Mond, Asteroid, Komet) haben die Objekte und welche Zusammensetzung besitzen sie?"
    ],
    "map": {
      "image": "Map.jpg",
      "referenceWidth": 1000,
      "referenceHeight": 1000
    },
    "apiResponse": {
      "id": "gen-1783863553-04clf6PQ9AjCGUJsVRSk",
      "created": 1783863553,
      "model": "x-ai/grok-4.20-20260309",
      "usage": {
        "prompt_tokens": 2431,
        "completion_tokens": 6134,
        "total_tokens": 8565,
        "cost": 0.01823935,
        "is_byok": false,
        "prompt_tokens_details": {
          "cached_tokens": 128,
          "cache_write_tokens": 0,
          "audio_tokens": 0,
          "video_tokens": 0
        },
        "cost_details": {
          "upstream_inference_cost": 0.01823935,
          "upstream_inference_prompt_cost": 0.00290435,
          "upstream_inference_completions_cost": 0.015335
        },
        "completion_tokens_details": {
          "reasoning_tokens": 0,
          "image_tokens": 0,
          "audio_tokens": 0
        }
      },
      "system_fingerprint": null
    }
  },
  "dataModel": {
    "entities": {
      "CelestialBody": {
        "properties": {
          "name": {
            "type": "string"
          },
          "type": {
            "type": "categorical",
            "values": [
              "Star",
              "Planet",
              "DwarfPlanet",
              "Moon",
              "Asteroid",
              "Comet"
            ]
          },
          "mass": {
            "type": "continuous",
            "range": [
              0,
              2000000
            ]
          },
          "diameter": {
            "type": "continuous",
            "range": [
              0,
              1500000
            ]
          },
          "surfaceTemperature": {
            "type": "continuous",
            "range": [
              -273,
              6000
            ]
          },
          "composition": {
            "type": "categorical",
            "values": [
              "Rocky",
              "Gaseous",
              "Icy",
              "Metallic",
              "Mixed"
            ]
          }
        }
      }
    },
    "relationships": {
      "Orbits": {
        "properties": {
          "semiMajorAxis": {
            "type": "continuous",
            "range": [
              0,
              50000000000
            ]
          },
          "orbitalPeriod": {
            "type": "continuous",
            "range": [
              0,
              1000000
            ]
          },
          "eccentricity": {
            "type": "continuous",
            "range": [
              0,
              1
            ]
          },
          "inclination": {
            "type": "continuous",
            "range": [
              0,
              180
            ]
          }
        }
      },
      "SatelliteOf": {
        "properties": {}
      },
      "DiscoveredBy": {
        "properties": {
          "discoveryYear": {
            "type": "continuous",
            "range": [
              1500,
              2025
            ]
          }
        }
      }
    }
  },
  "data": {
    "entities": [
      {
        "id": "sun",
        "type": "Star",
        "label": "Sun",
        "name": "Sun",
        "mass": 1988500,
        "diameter": 1392700,
        "surfaceTemperature": 5772,
        "composition": "Mixed",
        "position": {
          "x": 0,
          "y": 0,
          "z": 0
        },
        "mapX": 500,
        "mapY": 500,
        "temporal": {
          "validFrom": 4600,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "mercury",
        "type": "Planet",
        "label": "Mercury",
        "name": "Mercury",
        "mass": 330,
        "diameter": 4879,
        "surfaceTemperature": 167,
        "composition": "Rocky",
        "position": {
          "x": 8,
          "y": 2,
          "z": 0
        },
        "mapX": 620,
        "mapY": 480,
        "temporal": {
          "validFrom": 4600,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "venus",
        "type": "Planet",
        "label": "Venus",
        "name": "Venus",
        "mass": 4870,
        "diameter": 12104,
        "surfaceTemperature": 464,
        "composition": "Rocky",
        "position": {
          "x": 12,
          "y": 2,
          "z": 0
        },
        "mapX": 670,
        "mapY": 470,
        "temporal": {
          "validFrom": 4600,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "earth",
        "type": "Planet",
        "label": "Earth",
        "name": "Earth",
        "mass": 5972,
        "diameter": 12742,
        "surfaceTemperature": 15,
        "composition": "Rocky",
        "position": {
          "x": 18,
          "y": 2,
          "z": 0
        },
        "mapX": 720,
        "mapY": 500,
        "temporal": {
          "validFrom": 4600,
          "validTo": null,
          "history": [
            {
              "timestamp": 1800,
              "changes": {
                "surfaceTemperature": 14
              }
            }
          ]
        }
      },
      {
        "id": "mars",
        "type": "Planet",
        "label": "Mars",
        "name": "Mars",
        "mass": 639,
        "diameter": 6779,
        "surfaceTemperature": -65,
        "composition": "Rocky",
        "position": {
          "x": 24,
          "y": 2,
          "z": 2
        },
        "mapX": 790,
        "mapY": 510,
        "temporal": {
          "validFrom": 4600,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "jupiter",
        "type": "Planet",
        "label": "Jupiter",
        "name": "Jupiter",
        "mass": 1898600,
        "diameter": 139820,
        "surfaceTemperature": -110,
        "composition": "Gaseous",
        "position": {
          "x": -15,
          "y": 3,
          "z": 0
        },
        "mapX": 320,
        "mapY": 450,
        "temporal": {
          "validFrom": 4600,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "saturn",
        "type": "Planet",
        "label": "Saturn",
        "name": "Saturn",
        "mass": 568460,
        "diameter": 120536,
        "surfaceTemperature": -140,
        "composition": "Gaseous",
        "position": {
          "x": -22,
          "y": 3,
          "z": 5
        },
        "mapX": 240,
        "mapY": 420,
        "temporal": {
          "validFrom": 4600,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "uranus",
        "type": "Planet",
        "label": "Uranus",
        "name": "Uranus",
        "mass": 86832,
        "diameter": 51118,
        "surfaceTemperature": -195,
        "composition": "Gaseous",
        "position": {
          "x": -28,
          "y": 3,
          "z": -8
        },
        "mapX": 150,
        "mapY": 380,
        "temporal": {
          "validFrom": 4600,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "neptune",
        "type": "Planet",
        "label": "Neptune",
        "name": "Neptune",
        "mass": 102430,
        "diameter": 49528,
        "surfaceTemperature": -200,
        "composition": "Gaseous",
        "position": {
          "x": -32,
          "y": 3,
          "z": 10
        },
        "mapX": 80,
        "mapY": 350,
        "temporal": {
          "validFrom": 4600,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "pluto",
        "type": "DwarfPlanet",
        "label": "Pluto",
        "name": "Pluto",
        "mass": 13,
        "diameter": 2376,
        "surfaceTemperature": -225,
        "composition": "Icy",
        "position": {
          "x": -38,
          "y": 1,
          "z": 15
        },
        "mapX": 50,
        "mapY": 280,
        "temporal": {
          "validFrom": 1930,
          "validTo": null,
          "history": [
            {
              "timestamp": 2006,
              "changes": {
                "type": "DwarfPlanet"
              }
            }
          ]
        }
      },
      {
        "id": "moon",
        "type": "Moon",
        "label": "Moon",
        "name": "Moon",
        "mass": 73,
        "diameter": 3474,
        "surfaceTemperature": -20,
        "composition": "Rocky",
        "position": {
          "x": 19,
          "y": 4,
          "z": 1
        },
        "mapX": 740,
        "mapY": 520,
        "temporal": {
          "validFrom": 4600,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "io",
        "type": "Moon",
        "label": "Io",
        "name": "Io",
        "mass": 89,
        "diameter": 3643,
        "surfaceTemperature": -150,
        "composition": "Rocky",
        "position": {
          "x": -14,
          "y": 5,
          "z": 2
        },
        "mapX": 310,
        "mapY": 430,
        "temporal": {
          "validFrom": 1610,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "europa",
        "type": "Moon",
        "label": "Europa",
        "name": "Europa",
        "mass": 48,
        "diameter": 3122,
        "surfaceTemperature": -170,
        "composition": "Icy",
        "position": {
          "x": -16,
          "y": 5,
          "z": 1
        },
        "mapX": 300,
        "mapY": 440,
        "temporal": {
          "validFrom": 1610,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "ganymede",
        "type": "Moon",
        "label": "Ganymede",
        "name": "Ganymede",
        "mass": 148,
        "diameter": 5268,
        "surfaceTemperature": -160,
        "composition": "Icy",
        "position": {
          "x": -17,
          "y": 6,
          "z": 0
        },
        "mapX": 290,
        "mapY": 460,
        "temporal": {
          "validFrom": 1610,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "callisto",
        "type": "Moon",
        "label": "Callisto",
        "name": "Callisto",
        "mass": 108,
        "diameter": 4821,
        "surfaceTemperature": -140,
        "composition": "Icy",
        "position": {
          "x": -19,
          "y": 5,
          "z": -2
        },
        "mapX": 280,
        "mapY": 480,
        "temporal": {
          "validFrom": 1610,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "titan",
        "type": "Moon",
        "label": "Titan",
        "name": "Titan",
        "mass": 135,
        "diameter": 5150,
        "surfaceTemperature": -180,
        "composition": "Icy",
        "position": {
          "x": -23,
          "y": 5,
          "z": 6
        },
        "mapX": 220,
        "mapY": 400,
        "temporal": {
          "validFrom": 1655,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "ceres",
        "type": "DwarfPlanet",
        "label": "Ceres",
        "name": "Ceres",
        "mass": 0.94,
        "diameter": 939,
        "surfaceTemperature": -105,
        "composition": "Rocky",
        "position": {
          "x": 5,
          "y": 1,
          "z": -5
        },
        "mapX": 580,
        "mapY": 550,
        "temporal": {
          "validFrom": 1801,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "halley",
        "type": "Comet",
        "label": "Halley's Comet",
        "name": "Halley's Comet",
        "mass": 0.0002,
        "diameter": 11,
        "surfaceTemperature": -220,
        "composition": "Icy",
        "position": {
          "x": -10,
          "y": -8,
          "z": 25
        },
        "mapX": 410,
        "mapY": 150,
        "temporal": {
          "validFrom": 1758,
          "validTo": null,
          "history": []
        }
      },
      {
        "id": "vesta",
        "type": "Asteroid",
        "label": "Vesta",
        "name": "Vesta",
        "mass": 0.259,
        "diameter": 525,
        "surfaceTemperature": -100,
        "composition": "Rocky",
        "position": {
          "x": 3,
          "y": 0,
          "z": -8
        },
        "mapX": 570,
        "mapY": 570,
        "temporal": {
          "validFrom": 1807,
          "validTo": null,
          "history": []
        }
      }
    ],
    "relationships": [
      {
        "id": "rel_sun_mercury",
        "type": "Orbits",
        "source": "mercury",
        "target": "sun",
        "label": "Mercury orbits Sun",
        "semiMajorAxis": 57900000,
        "orbitalPeriod": 88,
        "eccentricity": 0.205,
        "inclination": 7,
        "temporal": {
          "validFrom": 4600,
          "validTo": null
        }
      },
      {
        "id": "rel_sun_venus",
        "type": "Orbits",
        "source": "venus",
        "target": "sun",
        "label": "Venus orbits Sun",
        "semiMajorAxis": 108200000,
        "orbitalPeriod": 225,
        "eccentricity": 0.007,
        "inclination": 3.4,
        "temporal": {
          "validFrom": 4600,
          "validTo": null
        }
      },
      {
        "id": "rel_sun_earth",
        "type": "Orbits",
        "source": "earth",
        "target": "sun",
        "label": "Earth orbits Sun",
        "semiMajorAxis": 149600000,
        "orbitalPeriod": 365,
        "eccentricity": 0.017,
        "inclination": 0,
        "temporal": {
          "validFrom": 4600,
          "validTo": null
        }
      },
      {
        "id": "rel_sun_mars",
        "type": "Orbits",
        "source": "mars",
        "target": "sun",
        "label": "Mars orbits Sun",
        "semiMajorAxis": 227900000,
        "orbitalPeriod": 687,
        "eccentricity": 0.094,
        "inclination": 1.85,
        "temporal": {
          "validFrom": 4600,
          "validTo": null
        }
      },
      {
        "id": "rel_sun_jupiter",
        "type": "Orbits",
        "source": "jupiter",
        "target": "sun",
        "label": "Jupiter orbits Sun",
        "semiMajorAxis": 778500000,
        "orbitalPeriod": 4333,
        "eccentricity": 0.049,
        "inclination": 1.3,
        "temporal": {
          "validFrom": 4600,
          "validTo": null
        }
      },
      {
        "id": "rel_sun_saturn",
        "type": "Orbits",
        "source": "saturn",
        "target": "sun",
        "label": "Saturn orbits Sun",
        "semiMajorAxis": 1433000000,
        "orbitalPeriod": 10759,
        "eccentricity": 0.057,
        "inclination": 2.5,
        "temporal": {
          "validFrom": 4600,
          "validTo": null
        }
      },
      {
        "id": "rel_sun_uranus",
        "type": "Orbits",
        "source": "uranus",
        "target": "sun",
        "label": "Uranus orbits Sun",
        "semiMajorAxis": 2871000000,
        "orbitalPeriod": 30687,
        "eccentricity": 0.046,
        "inclination": 0.8,
        "temporal": {
          "validFrom": 4600,
          "validTo": null
        }
      },
      {
        "id": "rel_sun_neptune",
        "type": "Orbits",
        "source": "neptune",
        "target": "sun",
        "label": "Neptune orbits Sun",
        "semiMajorAxis": 4495000000,
        "orbitalPeriod": 60190,
        "eccentricity": 0.011,
        "inclination": 1.77,
        "temporal": {
          "validFrom": 4600,
          "validTo": null
        }
      },
      {
        "id": "rel_sun_pluto",
        "type": "Orbits",
        "source": "pluto",
        "target": "sun",
        "label": "Pluto orbits Sun",
        "semiMajorAxis": 5906000000,
        "orbitalPeriod": 90560,
        "eccentricity": 0.244,
        "inclination": 17.2,
        "temporal": {
          "validFrom": 1930,
          "validTo": null
        }
      },
      {
        "id": "rel_earth_moon",
        "type": "Orbits",
        "source": "moon",
        "target": "earth",
        "label": "Moon orbits Earth",
        "semiMajorAxis": 384400,
        "orbitalPeriod": 27,
        "eccentricity": 0.055,
        "inclination": 5.1,
        "temporal": {
          "validFrom": 4600,
          "validTo": null
        }
      },
      {
        "id": "rel_jupiter_io",
        "type": "Orbits",
        "source": "io",
        "target": "jupiter",
        "label": "Io orbits Jupiter",
        "semiMajorAxis": 421700,
        "orbitalPeriod": 2,
        "eccentricity": 0.004,
        "inclination": 0.04,
        "temporal": {
          "validFrom": 1610,
          "validTo": null
        }
      },
      {
        "id": "rel_jupiter_europa",
        "type": "Orbits",
        "source": "europa",
        "target": "jupiter",
        "label": "Europa orbits Jupiter",
        "semiMajorAxis": 670900,
        "orbitalPeriod": 4,
        "eccentricity": 0.009,
        "inclination": 0.47,
        "temporal": {
          "validFrom": 1610,
          "validTo": null
        }
      },
      {
        "id": "rel_jupiter_ganymede",
        "type": "Orbits",
        "source": "ganymede",
        "target": "jupiter",
        "label": "Ganymede orbits Jupiter",
        "semiMajorAxis": 1070000,
        "orbitalPeriod": 7,
        "eccentricity": 0.001,
        "inclination": 0.17,
        "temporal": {
          "validFrom": 1610,
          "validTo": null
        }
      },
      {
        "id": "rel_jupiter_callisto",
        "type": "Orbits",
        "source": "callisto",
        "target": "jupiter",
        "label": "Callisto orbits Jupiter",
        "semiMajorAxis": 1883000,
        "orbitalPeriod": 17,
        "eccentricity": 0.007,
        "inclination": 0.19,
        "temporal": {
          "validFrom": 1610,
          "validTo": null
        }
      },
      {
        "id": "rel_saturn_titan",
        "type": "Orbits",
        "source": "titan",
        "target": "saturn",
        "label": "Titan orbits Saturn",
        "semiMajorAxis": 1222000,
        "orbitalPeriod": 16,
        "eccentricity": 0.029,
        "inclination": 0.33,
        "temporal": {
          "validFrom": 1655,
          "validTo": null
        }
      },
      {
        "id": "rel_satellite_earth_moon",
        "type": "SatelliteOf",
        "source": "moon",
        "target": "earth",
        "label": "Moon is satellite of Earth",
        "temporal": {
          "validFrom": 4600,
          "validTo": null
        }
      },
      {
        "id": "rel_satellite_io_jupiter",
        "type": "SatelliteOf",
        "source": "io",
        "target": "jupiter",
        "label": "Io is satellite of Jupiter",
        "temporal": {
          "validFrom": 1610,
          "validTo": null
        }
      },
      {
        "id": "rel_satellite_europa_jupiter",
        "type": "SatelliteOf",
        "source": "europa",
        "target": "jupiter",
        "label": "Europa is satellite of Jupiter",
        "temporal": {
          "validFrom": 1610,
          "validTo": null
        }
      },
      {
        "id": "rel_satellite_ganymede_jupiter",
        "type": "SatelliteOf",
        "source": "ganymede",
        "target": "jupiter",
        "label": "Ganymede is satellite of Jupiter",
        "temporal": {
          "validFrom": 1610,
          "validTo": null
        }
      },
      {
        "id": "rel_satellite_callisto_jupiter",
        "type": "SatelliteOf",
        "source": "callisto",
        "target": "jupiter",
        "label": "Callisto is satellite of Jupiter",
        "temporal": {
          "validFrom": 1610,
          "validTo": null
        }
      },
      {
        "id": "rel_satellite_titan_saturn",
        "type": "SatelliteOf",
        "source": "titan",
        "target": "saturn",
        "label": "Titan is satellite of Saturn",
        "temporal": {
          "validFrom": 1655,
          "validTo": null
        }
      },
      {
        "id": "rel_discovery_pluto",
        "type": "DiscoveredBy",
        "source": "pluto",
        "target": "sun",
        "label": "Pluto discovered",
        "discoveryYear": 1930,
        "temporal": {
          "validFrom": 1930,
          "validTo": null
        }
      },
      {
        "id": "rel_discovery_io",
        "type": "DiscoveredBy",
        "source": "io",
        "target": "jupiter",
        "label": "Io discovered",
        "discoveryYear": 1610,
        "temporal": {
          "validFrom": 1610,
          "validTo": null
        }
      },
      {
        "id": "rel_discovery_ceres",
        "type": "DiscoveredBy",
        "source": "ceres",
        "target": "sun",
        "label": "Ceres discovered",
        "discoveryYear": 1801,
        "temporal": {
          "validFrom": 1801,
          "validTo": null
        }
      },
      {
        "id": "rel_discovery_halley",
        "type": "DiscoveredBy",
        "source": "halley",
        "target": "sun",
        "label": "Halley's Comet discovered",
        "discoveryYear": 1758,
        "temporal": {
          "validFrom": 1758,
          "validTo": null
        }
      },
      {
        "id": "rel_discovery_vesta",
        "type": "DiscoveredBy",
        "source": "vesta",
        "target": "sun",
        "label": "Vesta discovered",
        "discoveryYear": 1807,
        "temporal": {
          "validFrom": 1807,
          "validTo": null
        }
      }
    ]
  }
}