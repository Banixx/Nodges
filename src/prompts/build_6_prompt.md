...
- RAEUMLICHKEIT & ZEITLICHKEIT: Wenn Position Werte existieren, mappe dies zwingend auf Position (`"position": { "source": "position", "function": "constant" }` im global_node visualMapping) und definiere `position` (Typ `spatial`) im `dataModel`. Gibt es zeitliche Entwicklungen (z.B. Epochen, Zeitstempel, Lebensdauer), nutze zwingend das optionale `temporal` Objekt bei Entities und Relationships (`"temporal": { "validFrom": <number>, "validTo": <number|null>, "history": [ { "timestamp": <number>, "changes": { "<property>": <newValue> } } ] }`). In `history` duerfen nur Deltas (geaenderte Properties) stehen. `validTo: null` bedeutet unbegrenzt.
