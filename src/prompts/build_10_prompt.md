...
- LIMITIERUNG: Bei dynamischen Size-Mappings darf die maximale Groesse maximal das 6-fache der minimalen Groesse betragen (z.B. min: 0.5, max: 3.0). Bei Kanten-Dicke (`thickness`) halte dich strikt an Werte zwischen 0.1 und 0.45.
- RAEUMLICHKEIT & ZEITLICHKEIT: Wenn du dich entscheidest, räumliche Positionen (`position` vom Typ `spatial`) zu generieren, erzeuge ECHTE, unterschiedliche Koordinaten für jeden Knoten (nicht nur 0,0,0). Mappe diese dann zwingend auf Position (`"position": { "source": "position", "function": "linear" }` im global_node visualMapping). Gibt es zeitliche Entwicklungen, nutze zwingend das optionale `temporal` Objekt bei Entities und Relationships (`"temporal": { "validFrom": <number>, "validTo": <number|null>, "history": [ { "timestamp": <number>, "changes": { "<property>": <newValue> } } ] }`). In `history` duerfen nur Deltas (geaenderte Properties) stehen. `validTo: null` bedeutet unbegrenzt.
...
- KANTEN: Mappe `color` kategorial auf das `type` (oder `label`) Attribut der Kanten (z. B. `"source": "type", "function": "categorical"`), damit unterschiedliche Kantenarten/Beziehungstypen in unterschiedlichen Farben dargestellt werden. Falls es ein durchgehendes numerisches Kanten-Property (z.B. "Intensitaet") gibt, mappe `thickness` darauf (limitiert auf 0.1 - 0.45), ansonsten setze `thickness` konstant (z. B. `"params": { "value": 0.1 }`).
