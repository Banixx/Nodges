...
- RAEUMLICHKEIT & ZEITLICHKEIT: Wenn du dich entscheidest, räumliche Positionen (`position` vom Typ `spatial`) zu generieren, erzeuge ECHTE, unterschiedliche Koordinaten für jeden Knoten (nicht nur 0,0,0). Mappe diese dann zwingend auf Position (`"position": { "source": "position", "function": "linear" }` im global_node visualMapping). Gibt es zeitliche Entwicklungen, ziehe diese fuer `animation` in Betracht.
