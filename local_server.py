#!/usr/bin/env python3
"""
Lokaler HTTP-Server mit CORS-Unterstuetzung fuer Webentwicklung
...
class CORSHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    """HTTP Request Handler mit CORS-Unterstuetzung"""
    
    def end_headers(self):
        """Fuegt CORS-Header zu allen Antworten hinzu"""
    ...
    def do_OPTIONS(self):
        """Behandelt OPTIONS-Requests fuer CORS-Preflight"""
...
def start_server(port=8000, directory=None):
    ...
    if directory:
        if os.path.exists(directory):
            os.chdir(directory)
            print(f"Arbeitsverzeichnis geaendert zu: {os.path.abspath(directory)}")
        else:
    ...
    try:
        ...
        with socketserver.TCPServer(("", port), CORSHTTPRequestHandler) as httpd:
            print(f"\n[START] CORS-faehiger HTTP-Server gestartet!")
            print(f"[FOLDER] Verzeichnis: {current_dir}")
            print(f"[WEB] URL: http://localhost:{port}")
            print(f"[LINK] Netzwerk-URL: http://{get_local_ip()}:{port}")
            print(f"\n[OK] CORS ist aktiviert - keine Cross-Origin-Probleme!")
            print(f"[STOP]  Zum Beenden: Ctrl+C druecken\n")
            
            # Server starten
            httpd.serve_forever()
            
    except KeyboardInterrupt:
        print("\n\n[BYE] Server wurde beendet.")
        return True
    except OSError as e:
        if e.errno == 48:  # Address already in use
            print(f"[ERROR] Fehler: Port {port} ist bereits in Verwendung!")
            print(f"[INFO] Versuchen Sie einen anderen Port: python {sys.argv[0]} {port + 1}")
        else:
            print(f"[ERROR] Fehler beim Starten des Servers: {e}")
        return False
    except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Lokaler HTTP-Server mit CORS-Unterstuetzung",
...
Beispiele:
  ...
  python local_server.py --port 5000 --dir ./build
    ...
    if port < 1 or port > 65535:
        print("[ERROR] Fehler: Port muss zwischen 1 und 65535 liegen!")
        return
